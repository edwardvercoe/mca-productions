...
const Image = styled.div((props) => [`background-image: url("${props.imageSrc}");`, tw`rounded md:w-1/2 lg:w-5/12 xl:w-1/3 flex-shrink-0 h-80 md:h-144 bg-contain bg-no-repeat bg-center mx-4 sm:mx-8 md:mx-4 lg:mx-8`]);
        ...
        <Content>
          {cards.map((card, i) => (
            <Card key={i} reversed={i % 2 === 1}>
              <Image imageSrc={"https:" + card.fields.supportingImage.fields.file.url} />
              <Details>
                ...
                <Link href={card.fields.videoUrl}>See Video</Link>
              </Details>
            </Card>
          ))}
        </Content>
